...
interface ServiceCenterAuthContextValue {
  serviceCenter: ServiceCenterUser | null;
  fetchServiceCenterMe: () => Promise<void>;
  register: (data: FormData) => Promise<any>;
  login: (data: ServiceCenterLoginDTO) => Promise<any>;
  handleforgotpassword: (email: string) => Promise<any>;
  handleResetPassword: (token: string, password: string) => Promise<any>;
  block: (id: string) => Promise<any>;
  logoutServiceCenter: () => Promise<void>;
  updateAvailability: (dto: AvailabilityFormData) => Promise<any>;
  fetchServices: (page: number, limit: number, search?: string) => Promise<void>;
  setErrors: React.Dispatch<React.SetStateAction<Record<string, string[]>>>;
  updateServiceCenterFee: (
    serviceId: string,
    advanceFee: number | null,
  ) => Promise<any>;
  handleAddservices: (dto: AddServiceDTO) => Promise<any>;
  fetchCategories: () => Promise<any>;
  toggleServiceStatu: (id: string) => Promise<any>;
...
const ServiceCenterAuthContext = createContext<
  ServiceCenterAuthContextValue | undefined
>(undefined);

export function ServiceCenterAuthProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const [serviceCenter, setServiceCenter] = useState<ServiceCenterUser | null>(
    null,
  );
  ...
  const fetchServiceCenterMe = useCallback(async () => {
    try {
      const res = await axiosClient.get(API_ROUTES.SERVICE_CENTER.PROFILE);
      setServiceCenter({
        id: res.data.data._id,
        garageName: res.data.data.providerProfile.garageName,
  ...
  return (
    <ServiceCenterAuthContext.Provider
      value={{
        serviceCenter,
        fetchServiceCenterMe,
        register,
...
export function useServiceCenterAuth() {
  const context = useContext(ServiceCenterAuthContext);
  if (!context) {
    throw new Error(
      "useServiceCenterAuth must be used within a ServiceCenterAuthProvider",
    );
  }
  return context;
}